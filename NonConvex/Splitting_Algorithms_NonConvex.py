...
from Strang_2D_NonConvex import Strang_2D
from CN_2D_NonConvex import CN_2D
from RK_2D_NonConvex import RK_2D
...
def E(u):
    return (2/3) * u[1] ** 2 + (2/3) * u[0] ** 2 + 3 * np.sin((u[1]+u[0])/np.sqrt(2)) ** 2
...
def gradE(u):
    return np.array([(4/3) * u[0] + (3/np.sqrt(2)) * np.sin(np.sqrt(2)*(u[0]+u[1])), (4/3) * u[1] + (3/np.sqrt(2)) * np.sin(np.sqrt(2)*(u[0]+u[1]))])
