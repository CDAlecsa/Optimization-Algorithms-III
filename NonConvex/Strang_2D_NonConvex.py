...
#    x[1] = nan_to_num(x[1])
    ...
    A = 1 / ( 2 * ( u_n[0] + u_n[1] - x[0] - x[1] ) )

    
    B = 3 * cos(sqrt(2)*x[0]) * cos(sqrt(2)*x[1]) - \
    3 * sin(sqrt(2)*x[0]) * sin(sqrt(2)*x[1]) - \
    3 * cos(sqrt(2)*u_n[0]) * cos(sqrt(2)*u_n[1]) + \
    3 * sin(sqrt(2)*u_n[0]) * sin(sqrt(2)*u_n[1])
    
    C1 = 2 * ( u_n[0] + x[0] ) * ( u_n[0] + u_n[1] - x[0] - x[1] )
    C2 = 2 * ( u_n[1] + x[1] ) * ( u_n[0] + u_n[1] - x[0] - x[1] )
...
def Strang_2D(h, u0, v0, E, gradE, E_min, eps=1e-8):
    ...
    while ok == 0:
        ...
        A = 1 / ( 2 * ( u[-2][0] + u[-2][1] - u[-1][0] - u[-1][1] ) )
    
        B = 3 * cos(sqrt(2)*u[-1][0]) * cos(sqrt(2)*u[-1][1]) - \
        3 * sin(sqrt(2)*u[-1][0]) * sin(sqrt(2)*u[-1][1]) - \
        3 * cos(sqrt(2)*u[-2][0]) * cos(sqrt(2)*u[-2][1]) + \
        3 * sin(sqrt(2)*u[-2][0]) * sin(sqrt(2)*u[-2][1])
    
        C1 = 2 * ( u[-2][0] + u[-1][0] ) * ( u[-2][0] + u[-2][1] - u[-1][0] - u[-1][1] )
        C2 = 2 * ( u[-2][1] + u[-1][1] ) * ( u[-2][0] + u[-2][1] - u[-1][0] - u[-1][1] )
